...
function validateProfileSchema() {
  ...
  return new Set((schema.markers || []).map(mk => mk.replace(/^\\/, "")));
}

function validateCase(model, reviewIds, allowedMarkers) {
  ...
  if (model.phenomena?.includes("compound")) {
    ...
    if (decomp?.components?.length) {
      caseCheck(byMarker("cf").length >= 1, "compound case with components lacks \\cf cross-references");
      caseCheck(lossNotes.some(v => /compound decomposition/.test(v)), "compound case lacks its \\nt model-loss marker");
    }
  }
...
const allowedMarkers = validateProfileSchema();
...
const items = models.map(model => validateCase(model, reviewIds, allowedMarkers));
