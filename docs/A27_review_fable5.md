...
## 6. Addendum (04-07-2026): post-serializer corpus re-sync

Between this review's merge ([PR #100](https://github.com/sanskrit-lexicon/csl-standards/pull/100)) and the same evening, the MDF serializer landed as [PR #99](https://github.com/sanskrit-lexicon/csl-standards/pull/99) — the §5 gate "once it lands" fired immediately: MDF became a measured lane (292 reports, all `lossy`), growing the loss corpus 1,430 → **1,722** and staling the just-merged paper numbers. A follow-up pass ([PR #101](https://github.com/sanskrit-lexicon/csl-standards/pull/101), Fable 5 `claude-fable-5`) re-synced everything to the measured corpus:

- **PAPER.md**: abstract/§2/§12 now state MDF as implemented and measured (not "serializer queued"); §4 re-tabulated (1,722 reports; 1007/640/75 = 58/37/4%; MDF row 0/0/292; causes 60/21/7/7/4%; phenomena 21/21/20%, five evidence phenomena **69%**); §1 reframed around the serialization standard (the one section PR #100's re-frame had left in the old two-model voice); §3 lists the MDF stage; §11 adds the in-pipeline MDF marker-profile validation (250/250 pass, 281 `
t` model-loss markers); §13/§14 updated; Bowers et al. now cited by DOI.
- **LOSS_ANALYSIS.md**: corpus header 1,722 / five families (target-model 876 = 292 symmetric probes × 3 lanes), MDF row + flat-target-asymmetry paragraph, cause/phenomenon tables and prose, reviewer-trust note (129 reviewed; MDF 33/33 lossy).
- **Figure 1** regenerated with the MDF box ([scripts/build-figures.mjs](https://github.com/sanskrit-lexicon/csl-standards/blob/main/scripts/build-figures.mjs) — the generator still drew a two-profile world; Figure 5 had already been regenerated by PR #99).
- The MDF lane's reports carry `extensionNeeded: false` throughout and name their `
t` markers in `mappedAs` — recorded, not remedied; the paper's remedy scope (§1, §10) now says so explicitly.

§12's scale-stability caveat inverted accordingly: the 500/1000 check now *predates the MDF lane* rather than MDF predating measurement.
