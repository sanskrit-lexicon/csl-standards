...
Date: 2026-06-20
Status: implementation handoff ready; no server implementation in this repository.
...
The first `csl-apidev` implementation slice should build only the MW REST pilot:
...
4. Return `xml: null` until TEI-P5 conversion lands; never place CSL display XML in the
   C-SALT `xml` slot.
5. Implement headword search modes first: `term`, `prefix`, `wildcard`, `regexp`, `fuzzy`.
...
For `csl-apidev`, Phase 1 validation should compare a fixed MW headword list against
